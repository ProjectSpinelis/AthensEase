...
public class App {
    ...
    public static RoutePlan generateDemoData() {
        ...
        */
        ...
        for (int i=0; i<3; i++) {
            cityList.add(new City(ProperListCreation.getUserSelectedLocations().get(i)));
        }
    ...
    private static void printRoutePlan(RoutePlan routePlan) {
        ...
        for (int i = 0; i < routePlan.getVisitList().size(); i++) {
            ...
            if (i > 0) {
                ...
                + routePlan.getVisitList().get(0).getCity().calculateDistanceToCity(new City("Athens")/*new City("athens - starting position", 37.9755648, 23.7348324))*/));
                totalDistance += routePlan.getVisitList().get(0).getCity().calculateDistanceToCity(new City("Athens")/*new City("athens - starting position", 37.9755648, 23.7348324)*/);
            }
        }
